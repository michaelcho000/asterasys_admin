# 2025년 10월 데이터 업로드 가이드

이 폴더는 2025년 10월 실적 CSV 원본을 보관하는 전용 위치입니다. 9월과 동일한 파일명을 유지해야 `npm run process-data -- --month=2025-10` 실행 시 자동으로 매핑됩니다.

## 1. 필수 CSV 목록
다음 파일 21개를 **한 번에** 업로드해주세요. 모든 파일은 UTF-8(CRLF 무관) 이어야 하며, 헤더 구조는 9월과 동일해야 합니다.
...
- 임시/백업 파일(`*.backup`, `~$*.csv`)은 `data/raw/archive/2025-10/`로 이동해주세요.
- YouTube 스크래퍼 JSON은 `data/raw/2025-10/dataset_youtube-scraper_YYYY-MM-DD_HH-mm-ss.json` 형태로 추가하면 됩니다.

## 2. 처리 순서
1. CSV 업로드 완료 후 다음 명령으로 파이프라인을 실행합니다.
   ```bash
   npm run process-data -- --month=2025-10
   npm run youtube:process -- --month=2025-10
   ```
2. 처리 완료 시 `data/processed/2025-10/`와 `data/raw/generated/2025-10/`에 결과물이 생성됩니다.
3. 처리 과정에서 최신 월이 자동으로 `config/latest-month.json`에 `"2025-10"`으로 갱신됩니다.

## 3. 검증 체크리스트
- `/api/data/files/cafe_rank?month=2025-10` 호출이 200을 반환하는지 확인합니다.
- 대시보드 헤더에서 2025년 10월을 선택했을 때 모든 위젯이 10월 데이터로 갱신되는지 점검합니다.
- 필요 시 9월 데이터를 재처리하려면 `npm run process-data -- --month=2025-09 --set-latest=false`를 사용합니다.

## 4. 참고
- 생성된 2차 CSV(`youtube_products.csv`, `youtube_market_share.csv`)는 `data/raw/generated/2025-10/`에 보존되며, 자동으로 `/api/data/youtube-*` 엔드포인트에서 참조됩니다.
- 업로드 후 이 README는 그대로 두고 실제 데이터만 교체하면 됩니다.



  - 사용자는 data/raw에 10월 CSV를 업로드하고, npm run process-data가 이를 파싱해 data/processed/*.json과 data/raw/generated/*.csv를 생성합니다.
  ...
  - 단일 월 하드코딩: processData와 여러 컴포넌트가 dataMonth: '2025-10' 같이 고정된 값을 사용합니다. 새로운 월 데이터를 추가하면 이전 값이 덮어써지거나 UI가 혼동될 가능성이 높습니다.
  ...
  - UI의 정적 비교 수치: src/utils/fackData/에 하드코딩된 값(예: 10월 판매량)이 여전히 여러 컴포넌트에 남아 있어 CSV 기반 값과 불일치가 발생할 수 있습니다.
  - 데이터 디렉터리 구조: 현재 data/raw 루트에 모든 CSV가 섞여 있고, 백업 파일까지 혼재합니다. 월이 추가되면 파일 수가 급증해 관리가 어려워집니다.

  9월·10월 동시 지원을 위한 단계별 계획(코드 작성 전 구상)

  3. API 확장
      - /api/data/files/[filename] 및 단일 CSV를 읽는 API에 ?month=2025-10 쿼리 파라미터를 추가해 해당 월 폴설계합니다.
  4. UI 월 필터링
      - 전역 상태나 컨텍스트로 “선택 월”을 관리하는 얇은 래퍼를 기획합니다. 각 컴포넌트는 month 값을 받아 API 호출 시 ?month=를 붙여 데이터를 가져오도록 합니다.
      - 대시보드 상단(또는 필터 패널)에 월 선택 드롭다운을 배치해 9월↔10월 데이터를 전환하도록 UX를 기획합니다. 기본 선택은 최신 월.
  5. 데이터 업로드·검증 흐름
      - 10월 CSV 업로드 시 data/raw/2025-10/에 동일한 파일 세트를 투입하고, npm run process-data -- --month=2025-10 실행 → generated/2025-10 산출 → API가 10월 데이터를 제공하는지 검증합니다.
      - 테스트 플랜: npm run process-data -- --month=2025-09으로 기존 월도 재처리해 두 월 모두 가용한지 확인합니다.
  6. 문서 및 운영 체크
      - DATA_SOURCES.md에 월별 폴더 구조와 실행 절차를 업데이트하고, 10월 CSV 명세(필드 추가/변경 여부)에 대한 검증 체크리스트를 준비합니다.
